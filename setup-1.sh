...
while getopts "ajstpdvh" opt ; do
    case $opt in
        ...
        d) DESKTOP="YES" ;;
        h)
            printf "Usage: setup.sh -a -j -s -t -p -d -v -h\n"
            printf "\t-a\tAll bar Server.\n"
            printf "\t-j\tJava\n"
            printf "\t-s\tServer\n"
            printf "\t-t\tTools\n"
...
if [ $ECLIPSE = "YES" ]; then
    aptitude $FLAGS eclipse
    aptitude $FLAGS eclipse-cdt
    aptitude $FLAGS eclipse-cdt-autotools
fi
